...
//            int a = Integer.parseInt(Integer.toString(str.charAt(x)));
            ...
            * Этот символ становится типом char, и сначала я написал просто "int a = Integer.parseInt(str.charAt(x));", а ИДЕЯ сказал обернуть в valueOf, потому что parseInt принимает string, а не char
            * Хотел обернуть в toString(), но у нас статик метод (потом сделал объект Main'a , но у меня начала херота передаваться, а не те цифры, что там есть
            * Потом, через операнд "+=" складываем сумму цифр
            * */
